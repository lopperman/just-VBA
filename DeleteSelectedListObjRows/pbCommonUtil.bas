...
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ '
Public Const tblSettingsLocal As String = "tblSettingsLocal"
...
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ '
    ...
    Public Function DeleteSelectedListObjRows( _
        ...
        protOptions = pbCommonUtil.CurrentProtectionOptions(lstObj)
        '--- --- ---'
...
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ '
''  Add Worksheets that will be ignored from Protection
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ '
Public Function AddProtectIgnore(ws As Worksheet)
    Dim Exists As Boolean, Item
    For Each Item In ignoreProtectSheets
        If StringsMatch(Item, ws.CodeName) Then
            Exists = True
            Exit For
        End If
    Next Item
    If Not Exists Then
        ignoreProtectSheets.Add ws.CodeName
    End If
End Function
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ '
''  Return TRUE if Worksheet ([ws]) should not be protected
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ '
Public Function IgnoreProtect(ws As Worksheet) As Boolean
    Dim Item
    For Each Item In ignoreProtectSheets
        If StringsMatch(Item, ws.CodeName) Then
            IgnoreProtect = True
...
Public Function UnprotectSheet(ByRef wksht As Worksheet _
    ...
    If Not IgnoreProtect(wksht) Then
...
Private Function ProtectSheet2(ByRef wksht As Worksheet _
    ...
    'If CurrentUser IsDev, and DEV_PAUSELOCKING setting = True, then
    'make sure worksheet is not protected
    If IsDEV Then
        If FileSetting(STG_DEV_PAUSELOCKING) = True Then
            If wksht.protectContents Then
                UnprotectSheet wksht, pwd:=CStr(password)
                Exit Function
            End If
        End If
    End If
...
'   WORKBOOK SETTINGS
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~
Public Function HasFileSetting(stgKey) As Boolean
    HasFileSetting = FileSettingIndex(stgKey) > 0
End Function
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~
'   Setting For MAC or PC
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~
Private Function AddSettingsRow(stgLstObj As ListObject) As Range
    With stgLstObj
        .Resize .Range.Resize(RowSize:=.Range.Rows.Count + 1)
        Set AddSettingsRow = .ListRows(.ListRows.Count).Range
    End With
End Function
Public Property Get FileSettingForOS(stgKey) As Variant
    'Adds "_MAC" for Mac, otherwise adds "_PC"
    #If Mac Then
        stgKey = ConcatWithDelim("_", stgKey, "MAC")
    #Else
        stgKey = ConcatWithDelim("_", stgKey, "PC")
    #End If
    FileSettingForOS = FileSetting(stgKey)
End Property
Public Property Let FileSettingForOS(stgKey, stgVal)
    #If Mac Then
        stgKey = ConcatWithDelim("_", stgKey, "MAC")
    #Else
        stgKey = ConcatWithDelim("_", stgKey, "PC")
    #End If
    Dim tIDX As Long, evts As Boolean
    evts = Application.EnableEvents
    Application.EnableEvents = False
    tIDX = FileSettingIndex(stgKey)
    If tIDX > 0 Then
        If Not StringsMatch(FileSetting(stgKey), stgVal) Then
            wt(tblSettingsLocal).ListRows(tIDX).Range(1, 2) = stgVal
            wt(tblSettingsLocal).ListRows(tIDX).Range(1, 3) = Now
        End If
    Else
        AddSettingsRow(wt(tblSettingsLocal)).Value = Array(stgKey, stgVal, Now)
    End If
    Application.EnableEvents = evts
End Property
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~
'   General Setting
' ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~ ~~~
Public Function GetSettingVal(stgKey, Optional stgType As SettingsType = SettingsType.stFileLevel)
    Select Case stgType
        Case SettingsType.stFileLevel
            GetSettingVal = FileSetting(stgKey)
        Case SettingsType.stComputerLevel
            RaiseError ERR_INVALID_SETTING_OPERATION, errorDesc:="SettingsType: " & stgType & " is not implemented!"
        Case Else
            RaiseError ERR_INVALID_SETTING_OPERATION, errorDesc:="SettingsType: " & stgType & " is not valid"
    End Select
End Function
Public Function SetSettingVal(stgKey, stgVal, Optional stgType As SettingsType = SettingsType.stFileLevel)
    Select Case stgType
        Case SettingsType.stFileLevel
            FileSetting(stgKey) = stgVal
        Case SettingsType.stComputerLevel
            RaiseError ERR_INVALID_SETTING_OPERATION, errorDesc:="SettingsType: " & stgType & " is not implemented!"
        Case Else
            RaiseError ERR_INVALID_SETTING_OPERATION, errorDesc:="SettingsType: " & stgType & " is not valid"
...
Public Property Get FileSetting(stgKey) As Variant
    Dim tIDX As Long
    tIDX = FileSettingIndex(stgKey)
    If tIDX > 0 Then
        FileSetting = wt(tblSettingsLocal).ListRows(tIDX).Range(1, 2)
    End If
End Property
Public Property Let FileSetting(stgKey, stgVal)
    Dim tIDX As Long, evts As Boolean
    evts = Application.EnableEvents
    Application.EnableEvents = False
    tIDX = FileSettingIndex(stgKey)
    ...
    Application.EnableEvents = evts
End Property
Public Property Get FileSettingUpdated(stgKey)
    Dim tIDX As Long
    tIDX = FileSettingIndex(stgKey)
    If tIDX > 0 Then
        FileSettingUpdated = wt(tblSettingsLocal).ListRows(tIDX).Range(1, 3)
    End If
End Property
Public Function FileSettingClear(stgKey)
    FileSetting(stgKey) = vbNullString
End Function
Public Function FileSettingDelete(stgKey) As Boolean
    Dim tIDX As Long, evts As Boolean
    evts = Application.EnableEvents
    Application.EnableEvents = False
    tIDX = FileSettingIndex(stgKey)
    If tIDX > 0 Then
        wt(tblSettingsLocal).ListRows(tIDX).Range.Delete xlShiftUp
    End If
    FileSettingDelete = HasFileSetting(stgKey) = False
    Application.EnableEvents = evts
End Function

Private Function FileSettingIndex(stgKey) As Long
    VerifySettings
    Dim lo As ListObject
    Set lo = wt(tblSettingsLocal)
    If lo Is Nothing Then
        RaiseError ERR_INVALID_SETTING_OPERATION, errorDesc:="ListObject: " & tblSettingsLocal & " is missing"
    Else
        With lo
            If .ListRows.Count > 1 Then
                On Error Resume Next
                FileSettingIndex = WorksheetFunction.XMatch(stgKey, .ListColumns("SettingKey").DataBodyRange, 0, 1)
                If Err.number <> 0 Then
                    Err.Clear
                End If
            End If
...
Private Function VerifySettings()
    If wt(tblSettingsLocal) Is Nothing Then
        Dim ws As Worksheet
        Set ws = ThisWorkbook.Worksheets.Add(After:=ThisWorkbook.Worksheets(ThisWorkbook.Worksheets.Count))
        ws.Name = "Settings"
        ws.Visible = xlSheetVeryHidden
        ws.Range("A5:C5").Value = Array("SettingKey", "SettingValue", "Updated")
        ws.Range("A6:C6").Value = Array("IsDev", "itfuture," & ENV_LogName & "," & ENV_User, Now)
        ws.Range("A7:C7").Value = Array("DEV_PAUSELOCKING", False, Now)
        Dim lo As ListObject
        Set lo = ws.ListObjects.Add(SourceType:=xlSrcRange, Source:=ws.Range("A5:C7"), XlListObjectHasHeaders:=xlYes)
        lo.Name = tblSettingsLocal
